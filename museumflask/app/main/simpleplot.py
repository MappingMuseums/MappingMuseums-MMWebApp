...
###### NEED TO CHANGE REWRITE IF YOU CHANGE THE URI, MAYBE BOTH IN CONFIG AND DEFINTIONS ###
	...
	definitions.HASNAME+"Year_existed"
        ]
    ptrsdict={}
    ptr=0
    for b in fields:
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def simpleplotView(self,propertytodisplay):
...
#   	    print "-----------------------------------------------------------"
        ...
        if (request.method == 'GET' and propertytodisplay != None):
		    ...
		    print str(e)
                    return render_template('message.html', title="Internal application error",message="Internal Application Error D-II detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
		    ...
		    print str(e)
                    return render_template('message.html', title="Internal application error",message="Internal Application Error E-II detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
		    ...
		    print str(e)
                    return render_template('message.html', title="Internal application error",message="Internal Application Error F-II detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
                
                treetext=self.delegate_countyproperty(definitions.COUNTY,results)
		print "?#?#? simpleplot.py at line: 169 Dbg-out variable \treetext [",treetext,"]\n";
            #else:

	    
	    print "?#?#? simpleplot.py at line: 180 Dbg-out variable \propertytodisplay [",propertytodisplay,"]\n";
	    ...
	    #print "?#?#? simpleplot.py at line: 158 Dbg-out variable \dim [",dim,"]\n";
	    
	    print "?#?#? simpleplot.py at line: 190 Dbg-out variable \plotoptions [",self.plotoptlist,"]\n";
            return render_template('simpleplot.html',
				   dim=dim,
				   loc=self.ShowMuseumTypes.getLocationTree(),
				   dicts=dictdata,
				   plotoptions=self.plotoptlist,
				   xoptions=self.xoptlist,
				   yoptions=self.yoptlist,
				   mapdata=mapdata,
				   results=None)
