...
#				   trees=Museum.getConfiguration(),
#				   dicts=Museum.dictdata,
#				   mapdata=Museum.mapdata,
#				   results=None)
#
#
...
#-  -  -  -  -  -  -  
	...
	return Museum.menutree

    @staticmethod
    def getLocationTree():
	return Museum.locationtree

    @staticmethod
    def getDimensionTree():
	return Museum.dimensiontree

    @staticmethod
    def getPtrsDict():
	return Museum.ptrsdict

    @staticmethod
    def getMuseumDict():
	return Museum.treedict
    

#-  -  -  -  -  -  -  
    def removeStopWords(self,sname):
	for word in Museum.stopwords:
...
## Purpose:Initialises models from previously pickled build
	...
	menut=tree.Tree(None,False,False,"menu")
        Museum.menutree      = menut.loadTree("menut")
	Museum.ptrsdict      = self.loadDict("ptrsdict")
	Museum.treedict      = self.loadDict("treedict")
...
#  each property(column) to include in the menu.
	...
	for col in DEFAULT_BROWSE_COLUMNS:
		...
		if ("sortoption" in options):
		    ...
		    print("method")
		    ##print(method)
		    if (callable(getattr(Museum.customsort, method))):
			print("callable")
			func = getattr(Museum.customsort, method)
		...
		try:
		    ...
		    print "?#?#? museum.py at line: 498 Dbg-out variable \props [",props,"]\n";
		    results=apputils.getMarkerData(props)
		except Exception, e:
		    print str(e)
	...
	try:
	    ...
	    print "?#?#? museum.py at line: 582 Dbg-out variable \props [",props,"]\n";
 	    results=apputils.getMarkerData(props)
	except Exception, e:
	    print str(e)
	...
	try:
	    ...
	    print "?#?#? museum.py at line: 599 Dbg-out variable \props [",props,"]\n";
 	    results=apputils.getMarkerData(props)
	except Exception, e:
	    print str(e)
	...
	#### END Handling of generic properties 2  
 	...
 	##try:
	    ...
	    ##print "?#?#? museum.py at line: 616 Dbg-out variable \props [",props,"]\n";
 	    ##results=apputils.getMarkerData(props)
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def museumView(self,musid):
        print musid
        properties=apputils.getMuseumPropertiesForId(musid)
        print properties["results"]["bindings"]
        if(properties["results"]["bindings"]==[]):
            musname = "No Museum matches this url"
            return render_template('/Museum.html',
								 properties=[],
                                 musname = musname)
        propnames=[]
        propvalues=[]
        for item in properties["results"]["bindings"]:
            item.pop('museum') 
            for f in item:
                print f
                if("http://bbk.ac.uk/MuseumMapProject/def/" in item[f]["value"]):
                    print "yes"
                    item[f]["value"]=item[f]["value"].replace("http://bbk.ac.uk/MuseumMapProject/def/", "")
                
                if (f=='Name_of_museum'):
                    musname = item[f]["value"]
                
                     
                if(f not in propnames):
                    propnames.append(f)
                    ##print item[f]["value"]
                    propvalues.append(item[f]["value"])
        propnames, propvalues = zip(*sorted(zip(propnames, propvalues)))
        return render_template('/Museum.html',
								 properties=zip(propnames,propvalues),
                                 musname = musname)
