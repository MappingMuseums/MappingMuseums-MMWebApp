...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    def plotTopic(self,topic,inyear):
	plotdata=[]
	plotdata.append("const data = [")


 	try:
	    if (inyear == "Year_existed"):
		props=[topic,definitions.YEAR_OPENED,definitions.YEAR_CLOSED]
	    else:
		props=[topic,inyear]
 	    results=apputils.getMarkerData(props)
 	except Exception, e:
 	    print str(e)
 	    return render_template('message.html', title="Internal application error",message="Internal Application Error G detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")

        print len(results["results"]["bindings"])
	print "?#?#? plot.py at line: 71 Dbg-out variable \inyear [",inyear,"]\n";
	
	score={}

	## Preprocess the data for existence
	if (inyear == "Year_existed"):
	    musdict={}
	    firstyear=99999
	    lastyear=1
		...
		if (topic  in res):
		    ...
		    closeyear=""
		    sub=res[topic]["value"]
		    #print "?#?#? plot.py at line: 81 Dbg-out variable \sub [",sub,"]\n";
		    ...
		    if (openyear != ""):
			if (int(openyear) < firstyear):
			    firstyear=int(openyear)
			if (closeyear != ""):
			    musdict[uri]=(int(openyear),int(closeyear),sub)
			    #print "?#?#? plot.py at line: 91 Dbg-out variable \musdict [",musdict[uri],"]\n";
			    if (int(closeyear) >  lastyear):
				lastyear=int(closeyear)
			else:
			    musdict[uri]=(int(openyear),9999,sub)
			    #print "?#?#? plot.py at line: 96 Dbg-out variable \musdict [",musdict[uri],"]\n";
		...
		for key, val in musdict.iteritems():
		    ...
		    if (openyear <= firstyear and closeyear >= firstyear):  
			compokey=sub + ":" + str(firstyear)
	...
	# Done
		...
		if (topic  in res):
		    sub=res[topic]["value"]
		    if (inyear in res and int(res[inyear]["value"]) >= 1960):
	...
	for ltuple in sorted(tuplist, key=lambda x: x[0]):
	    year,sub,val=ltuple
	    line='[ country: "{}",year:"{}", value:{} ],'.format(sub,year,val)
	...
	results=[]
	score=[]
	plotdata[-1]=plotdata[-1].replace("},","}")
	plotdata.append("];")
	return plotdata
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

    def plotTopicPie(self,topic):
	plotdata=[]
	plotdata.append("const data = [")
 	...
 	try:
	    props=[topic]
 	    ...
 	    print str(e)
 	    return render_template('message.html', title="Internal application error",
				   message="Internal Application Error H detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
	...
	for key, val in score.iteritems():
	    total=total+val
	#print "?#?#? plot.py at line: 136 Dbg-out variable \total [",total,"]\n";

	for key, val in score.iteritems():
	    percent=(val/float(total))*100
	    line="[ item: '{}',count:{}],".format(key,val)
	    line=line.replace("[","{").replace("]","}")
	    plotdata.append(line)
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    def plotTopicTree(self,topic):
	plotdata=[]
	plotdata.append("const data = {")
	plotdata.append("name: 'root',")
	plotdata.append("children: [")
 	...
 	try:
	    props=[topic]
 	    ...
 	    print str(e)
 	    return render_template('message.html', title="Internal application error",message="Internal Application Error I detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
...
#	print "?#?#? plot.py at line: 136 Dbg-out variable \total [",total,"]\n";
	...
	for key, val in score.iteritems():
	    percent=(val/float(total))*100
	    line="[ name: '{}',value:{}],".format(key,val)
	    line=line.replace("[","{").replace("]","}")
	    plotdata.append(line)
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    def plotTopicBubble(self,topic):
	plotdata=[]
	x=request.form["xaxisselect"]

	
 	try:
	    props=[definitions.DOMUS_SUBJECTCLASSIFICATION,x,definitions.NAME_OF_MUSEUM,definitions.VISITORNUMBERS]
	    props=[definitions.VISITORNUMBERS,definitions.NAME_OF_MUSEUM,definitions.DOMUS_SUBJECTCLASSIFICATION]
 	    results=apputils.getMarkerData(props)
 	except Exception, e:
 	    print str(e)
 	    return render_template('message.html', title="Internal application error",message="Internal Application Error J detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
	...
	for res in results["results"]["bindings"]:
	    ...
	    if (definitions.DOMUS_SUBJECTCLASSIFICATION  in res and
		#x      in res and
		definitions.NAME_OF_MUSEUM in res and
		definitions.VISITORNUMBERS in res):
		#
		sub=res[definitions.DOMUS_SUBJECTCLASSIFICATION]["value"]
		name=res[definitions.NAME_OF_MUSEUM]["value"]
		visit=res[definitions.VISITORNUMBERS]["value"]
		visitornumber=visit.split(":")
		
		#year=res[x]["value"]
		year=random.randint(00,99)		
		line='{ "Subject": "'+sub+'","MuseumName":"'+name+'", "OpeningYear":'+str(year)+', "VisitorNumbers":'+str(visitornumber[1])+' },'
		museums[name]=line
		categ[sub]=sub


	for key,val in museums.items():
	    plotdata.append(val)
	count=0
	categdata.append("  const colorMap = {")
	for key,val in categ.items():
	    #print val
	    #print "'"+val+"': Global.colors["+str(count)+"],"
	    categdata.append("'"+val+"': Global.colors["+str(count)+"],")
	    count+=1
	    
	plotdata[-1]=plotdata[-1].replace("},","}")
	categdata[-1]=categdata[-1].replace("],","]")
	plotdata.append("];")
	categdata.append("};")

        
        return plotdata,categdata
	
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

    def plotTopicLine(self,topic,inyear):

	plotdata=[]
	plotdata.append("const chartData = [")
	...
	linedata=[]
 	...
 	try:
		...
		props=[topic,inyear]
 	    ...
 	    print str(e)
 	    return render_template('message.html', title="Internal application error",message="Internal Application Error K detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
...
#       get all topics
	...
	for res in results["results"]["bindings"]:
		...
		if (openyear != ""):
			...
			firstyear=int(openyear)
		    if (closeyear != ""):
			line='[ openyear: "{}",closeyear:"{}", sub:{} ],'.format(openyear,closeyear,sub)
			print line
			musdict[uri]=(int(openyear),int(closeyear),sub)
			#print "?#?#? plot.py at line: 91 Dbg-out variable \musdict [",musdict[uri],"]\n";
			if (int(closeyear) >  lastyear):
			...
			line='[ openyear: "{}",closeyear:"{}", sub:{} ],'.format(openyear,9999,sub)
			print line
			musdict[uri]=(int(openyear),9999,sub)
			#print "?#?#? plot.py at line: 96 Dbg-out variable \musdict [",musdict[uri],"]\n";
		...
		for key, val in musdict.iteritems():
			...
			sub=parts[0]
			alltopics[str(sub)]=1
			compokey=str(openyear)
			if (compokey in score):
			    if (sub in score[compokey]):
				score[compokey][sub]=score[compokey][sub]+1
			    else:
				score[compokey][sub]=1
			else:
			    score[compokey]=copy.deepcopy(alltopics)
			    score[compokey][sub]=1
	...
	maxval=-1
	sortedindex=sorted(score)
	tuplist=[]
	for year in sortedindex:
	    tdict=score[str(year)]
	    line="{year:'"+str(year)+"',"
	    for skey, sval in tdict.iteritems():
		line=line+str(skey)+":"+str(sval)+","
		if (sval > maxval):
		    maxval=sval
	    line=line+"},"
	    line=line.replace(",}","}")
    ...
    chart.line().position('year*${topic}').shape('dot')
      .color('${colour}');
    chart.point().position('year*${topic}').size(6)
      .shape('circle')
      .style({
        lineWidth: 2,
        stroke: '#fff'
...
"""	
	...
	print "--------------------------------------------------------"
	cptr=0
	for skey in alltopics.iteritems():
	    t,n=skey
	    #print "?#?#? plot.py at line: 220 Dbg-out variable \n [",n,"]\n";
	    #print "?#?#? plot.py at line: 220 Dbg-out variable \t [",t,"]\n";
	    draw=drawline.replace("${topic}",str(t)).replace("${colour}",str(colors[(cptr % len(colors))]))
	    linedata.append(draw)
	    cptr+=1
	    #print draw
	    #print skey
...
#	for l in linedata:
#	    print l
	return plotdata,linedata,maxval
	
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 


    def plotTopicLineOrig(self,topic,inyear):
	...
	linedata=[]
 	...
 	try:
	    props=[topic,inyear]
 	    ...
 	    print str(e)
 	    return render_template('message.html', title="Internal application error",message="Internal Application Error L detected. The Mapping Museums team would be very grateful if you could please use Get in Touch, under the Contact Us tab, to describe the actions that you took that led to this error message")
...
#    const data = [
#      17     { item: 'ESC$B;vNc0lESC(B', count: 40 },
#      18     { item: 'ESC$B;vNcFsESC(B', count: 21 },
#      19     { item: 'ESC$B;vNc;0ESC(B', count: 17 },
#      20     { item: 'ESC$B;vNc;MESC(B', count: 13 },
#      21     { item: 'ESC$B;vNc8^ESC(B', count: 9 }
#      22   ];
...
#    chart.line().position('year*cars').shape('dot')
#       .color('#0067e3');
#     chart.point().position('year*cars').size(6)
#       .shape('square')
#       .style({
#         lineWidth: 2,
#         stroke: '#fff'
...
#    chart.line().position('year*motorcycles').shape('circle')
#       .color('#0037e3');
#     chart.point().position('year*motorcycles').size(6)
#       .shape('circle')
#       .style({
#         lineWidth: 2,
#         stroke: '#fff'
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	...
	p=request.form["plotselect"]
	x=request.form["xaxisselect"]
	y=request.form["yaxisselect"]

	plotdata,linedata,maxval=self.plotTopicLine(y,x)
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    def plotView(self):
        ...
        if (request.method == 'GET'):
            return render_template('plot.html',  attributes=(definitions.DEFAULT_PLOT_FILTER_COLUMNS),attritypes=definitions.ATTRITYPES,datagroups=definitions.DATAGROUPS)
	...
	y=request.form["yaxisselect"]
        ...
        plotmode=request.form['plotmode']
	print "?#?#? plot.py at line: 68 Dbg-out variable plotmode [",plotmode,"]\n";
	if (plotmode == "simple"):
	    if (p == 'Bar'):
		plotdata=self.plotBar()
		return render_template('plotImpl/stacked_columns.html', title="Stacked columns",plotdata=plotdata)
	    elif (p == 'Line'):
		plotdata,linedata,maxval=self.plotLine()
		return render_template('plotImpl/line.html', title="Line graph",plotdata=plotdata,linedata=linedata,maxval=maxval)
	    elif (p == 'Pie'):
		plotdata=self.plotTopicPie(y)
		return render_template('plotImpl/pie.html', title="Pie graph",plotdata=plotdata)
	    elif (p == 'Tree'):
		plotdata=self.plotTopicTree(y)
		return render_template('plotImpl/tree.html', title="Tree graph",plotdata=plotdata)
	    elif (p == 'Bubble'):
		plotdata,categdata=self.plotTopicBubble(y)
		#for p in plotdata:
		#    print p
		return render_template('plotImpl/bubble.html', title="Bubble graph",plotdata=plotdata,categdata=categdata)
		
	else:
	    plotlist=request.form.getlist('plotlist[]')
	    print "?#?#? plot.py at line: 60 Dbg-out variable \plotlist [",plotlist,"]\n";
	    dimlist=request.form.getlist('dimlist[]')
	    temporal=request.form.getlist('temporal')
	    print "?#?#? plot.py at line: 62 Dbg-out variable \dimlist [",dimlist,"]\n";
	    return render_template('searchmessage.html', title="Plot Params",message="plotlist ["+str(plotlist)+"]\n dimlist ["+str(dimlist)+"]\n temporal:"+str(temporal))
